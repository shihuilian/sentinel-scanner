...
一个 Web 应用安全扫描器，覆盖 OWASP Top 10 里常见的几类漏洞，分被动审计和主动注入两条线。后端用 Node 原生模块写，前端是原生 HTML/CSS/JS，直接 `node server/index.js` 起服务，默认 4000 端口。

## 检测能力

- 被动：安全头、Cookie 标志、TLS 证书、敏感信息泄露、端点暴露、CSRF、开放重定向、CORS、路径遍历、JWT
- 主动：反射型 XSS、SQL 注入、SSRF、XXE
- 认证态扫描：带上会话 Cookie（可一键登录内置靶机）后，把登录后的页面也纳入检测

仓库里带了一个 `/demo` 靶机，里面故意留了上面这些漏洞，本地试的时候直接扫它。
...
只用来扫你拥有或已获书面授权的目标，以及本地这个 `/demo` 靶机。扫别人资产前必须拿到授权；未授权扫描可能违反《计算机信息系统安全保护条例》及相应法律。

---

_备注：默认扫描本地 /demo 靶机；扫外部域名前请确认已获授权。_
